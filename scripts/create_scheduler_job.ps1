...
# Check if the job already exists and update, otherwise create
$existingJob = gcloud scheduler jobs describe ads-fetcher-cron --location=$Region --project=$Project 2>&1
if ($LASTEXITCODE -eq 0) {
    Write-Output "Scheduler job 'ads-fetcher-cron' already exists - updating..."
    gcloud scheduler jobs update http ads-fetcher-cron `
        ...
        --location=$Region `
        --project=$Project
    Write-Output "Scheduler job updated."
} else {
    Write-Output "Creating Cloud Scheduler job 'ads-fetcher-cron'..."
    gcloud scheduler jobs create http ads-fetcher-cron `
        --schedule="$Schedule" `
        --uri="$SERVICE_URL/run" `
        --http-method=POST `
        --message-body='{}' `
        ...
        --location=$Region `
        --project=$Project
    Write-Output "Scheduler job created."
}

Write-Output "Done. View job: gcloud scheduler jobs describe ads-fetcher-cron --location=$Region --project=$Project"